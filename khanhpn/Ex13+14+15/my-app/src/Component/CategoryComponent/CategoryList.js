...
const CategoryList = (props) => {
    ...
    let listCategory = 
    category
    .map(x => <CategoryItem className="categoryitem" data={x} key={x.id} clickMe={showCategoryDetail} deleteMe={deleteCategory}/>)
        ...
        return(
            <div>
                ...
                onChange={(x)=>{setPage(x);onPageChange(x-1)}} onChangeSize={(x)=>{setSizePage(x);setPage(1)}}/>
            </div>
